...
const previewSheets = [
  { name: "Sidebar Moderne", presetId: "sidebar", data: exampleCVs["dev-fullstack"].data },
  { name: "Deux Colonnes", presetId: "two-columns", data: exampleCVs["data-scientist"].data },
  { name: "Créatif Coloré", presetId: "creative", data: exampleCVs["ux-designer"].data },
];

/** Vignette A4 d'un modèle canvas, rendue avec un CV d'exemple rempli. */
const PreviewSheet = ({ name, presetId, data }: typeof previewSheets[number]) => {
  const preset = getPreset(presetId);
  const doc = preset.build(preset.accent);
  const scale = 210 / PAGE_WIDTH;

  return (
    <div className="group relative aspect-[0.7] w-full max-w-[210px] overflow-hidden rounded-md border border-slate-200 bg-white shadow-lg transition duration-500 hover:-translate-y-2 hover:shadow-2xl">
      <div className="absolute inset-0 overflow-hidden bg-slate-50 transition-transform duration-500 group-hover:scale-[1.04]">
...
const Index = () => {
  ...
  >
    <PreviewSheet {...sheet} />
      ...
      <section id="modeles" className="mx-auto max-w-6xl px-5 py-16 text-center sm:px-8 sm:py-24"><p className="text-xs font-bold uppercase tracking-[0.2em] text-emerald-700">Une collection pensée pour vous</p><h2 className="mt-3 text-3xl font-bold tracking-tight sm:text-4xl">Des modèles qui donnent envie de vous rencontrer</h2><p className="mx-auto mt-4 max-w-xl text-sm leading-6 text-slate-600">Choisissez une base, personnalisez chaque détail et gardez le contrôle de votre candidature.</p><div className="mt-10 grid gap-4 sm:grid-cols-3">{previewSheets.map((sheet, index) => (
  <Reveal key={sheet.name} delay={index * 110} from="scale">
    <button
      onClick={() => navigate("/editeur", { state: { presetId: sheet.presetId } })}
      className="w-full rounded-lg border border-slate-200 bg-white p-4 text-left transition duration-300 hover:-translate-y-1 hover:border-emerald-300 hover:shadow-xl"
    >
      <PreviewSheet {...sheet} />
      <div className="mt-4 flex items-center justify-between">
        <span className="font-bold">{sheet.name}</span>
        <span className="text-xs font-semibold text-emerald-700">Personnaliser <ArrowRight className="ml-1 inline h-3 w-3 transition-transform group-hover:translate-x-1" /></span>
      </div>
    </button>
  </Reveal>
))}</div><button onClick={() => navigate("/modeles")} className="mt-9 rounded-md border border-slate-300 bg-white px-5 py-2.5 text-sm font-bold text-slate-700 hover:border-slate-500">Explorer tous les modèles</button></section>
