...
export const TemplatePhotoSidebar = ({ cvData }: { cvData: CVData }) => {
      ...
      <div className="flex" style={{ fontFamily: 'system-ui, -apple-system, sans-serif' }}>
        ...
        >
          ...
          <div className="space-y-3 mb-6 text-sm">
            {cvData.email && (
              <div className="flex items-center gap-2">
                ...
                <span className="text-white/90 text-xs">{safeValue(cvData.email)}</span>
              </div>
            )}
            {cvData.phone && (
              <div className="flex items-center gap-2">
                <Phone className="w-4 h-4 flex-shrink-0" />
                <span className="text-white/90 text-xs">{safeValue(cvData.phone)}</span>
              </div>
            )}
            {cvData.address && (
              <div className="flex items-center gap-2">
                <MapPin className="w-4 h-4 flex-shrink-0" />
                <span className="text-white/90 text-xs">{safeValue(cvData.address)}</span>
              </div>
            )}
          </div>
          ...
          {(cvData.linkedin || cvData.github || cvData.twitter || cvData.portfolio) && (
            <div className="mb-6">
              <h3 className="text-xs uppercase tracking-widest font-bold mb-3">Réseaux</h3>
          ...
          {cvData.skills.length > 0 && (
            <div>
              <h3 className="text-xs uppercase tracking-widest font-bold mb-3">Compétences</h3>
              <div className="space-y-3">
                {cvData.skills.map((skill) => (
                  <div key={skill.id}>
                    <div className="flex justify-between items-center mb-1">
                      <span className="text-xs text-white/90">{safeValue(skill.name)}</span>
