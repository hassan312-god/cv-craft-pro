...
interface TemplateDarkSidebarProps {
  cvData: CVData;
}

export const TemplateDarkSidebar = ({ cvData }: TemplateDarkSidebarProps) => {
    ...
    <Card className="overflow-hidden border border-border">
      <div className="flex" style={{ fontFamily: 'system-ui, -apple-system, sans-serif' }}>
        ...
        >
          {/* Name Header */}
          <div className="mb-6">
            <h1 className="text-2xl font-bold mb-1 uppercase leading-tight">
              {safeValue(cvData.firstName)} {safeValue(cvData.lastName)}
            </h1>
            <div className="h-px w-16 bg-white mt-2 mb-4" />
          </div>
          ...
          <div className="space-y-3 mb-6 text-sm">
            {cvData.email && (
              <div className="flex items-center gap-2">
                ...
                <span className="text-white/90">{safeValue(cvData.email)}</span>
              </div>
            )}
            {cvData.phone && (
              <div className="flex items-center gap-2">
                <Phone className="w-4 h-4 flex-shrink-0" />
                <span className="text-white/90">{safeValue(cvData.phone)}</span>
              </div>
            )}
            {cvData.address && (
              <div className="flex items-center gap-2">
                <MapPin className="w-4 h-4 flex-shrink-0" />
                <span className="text-white/90">{safeValue(cvData.address)}</span>
              </div>
            )}
          </div>
          ...
          {(cvData.linkedin || cvData.github || cvData.twitter || cvData.portfolio) && (
            <div className="mb-6">
              <h2 className="text-sm font-bold mb-3 text-white uppercase">RÉSEAUX</h2>
          ...
          {cvData.skills.length > 0 && (
            <div>
              <h2 className="text-sm font-bold mb-3 text-white uppercase">COMPÉTENCES</h2>
              <div className="space-y-3">
                {cvData.skills.map((skill) => (
                  <div key={skill.id}>
                    <div className="flex justify-between items-center mb-1">
                      <span className="text-xs text-white/90 uppercase">{safeValue(skill.name)}</span>
                ...
                {cvData.experiences.map((exp) => (
                  <div key={exp.id}>
                    <div className="flex justify-between items-start mb-1">
                      <div>
                        <h3 className="font-bold text-base uppercase" style={{ color: colors.primary }}>
            ...
            <div>
              ...
              <div className="space-y-4">
                {cvData.education.map((edu) => (
                  <div key={edu.id}>
                    <h3 className="font-bold text-sm uppercase" style={{ color: colors.primary }}>
