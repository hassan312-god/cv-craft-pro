...
export const TemplateColorful = ({ cvData }: { cvData: CVData }) => {
  ...
  return (
    <Card className="overflow-hidden border-0 shadow-none" style={{ width: '794px' }}>
      <div className="bg-white" style={{ fontFamily }}>
        ...
        <div className="p-10">
          {/* Contact avec icônes colorées */}
          <div className="grid grid-cols-3 gap-4 mb-8 text-sm">
            {cvData.email && (
              <div className="flex items-center gap-2">
                <div className="p-2 rounded" style={{ backgroundColor: colors.light }}>
                  <Mail className="w-4 h-4" style={{ color: colors.primary }} />
                </div>
                <span style={{ color: colors.text }}>{safeValue(cvData.email)}</span>
              </div>
            )}
            {cvData.phone && (
              <div className="flex items-center gap-2">
                ...
                <span style={{ color: colors.text }}>{safeValue(cvData.phone)}</span>
              </div>
            )}
            {cvData.address && (
              <div className="flex items-center gap-2">
                ...
                <span style={{ color: colors.text }}>{safeValue(cvData.address)}</span>
              </div>
            )}
          </div>
            ...
            <div className="mb-8">
              ...
              <div className="space-y-5">
                {cvData.experiences.map((exp) => (
                  <div key={exp.id} className="border-l-4 pl-4" style={{ borderColor: colors.primary }}>
            ...
            <div className="mb-8">
              ...
              <div className="space-y-4">
                {cvData.education.map((edu) => (
                  <div key={edu.id} className="border-l-4 pl-4" style={{ borderColor: colors.accent }}>
                    <h3 className="font-bold text-base" style={{ color: colors.primary }}>
